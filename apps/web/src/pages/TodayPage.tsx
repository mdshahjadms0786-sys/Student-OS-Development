...
export function TodayPage() {
  ...
  const fetchData = React.useCallback(async (d: Date) => {
    setLoading(true);
    ...
    try {
      const [timetableRes, tasksRes] = await Promise.all([
        apiClient<TimetableEntry[]>(`/api/timetable?dayOfWeek=${dayOfWeek}`),
        apiClient<TodayTask[]>(`/api/tasks?from=${startStr}&to=${endStr}`),
      ]);
  ...
  const handleToggleTask = async (taskId: string) => {
    try {
      const res = await apiClient<TodayTask>(`/api/tasks/${taskId}/complete`, { method: 'POST' });
