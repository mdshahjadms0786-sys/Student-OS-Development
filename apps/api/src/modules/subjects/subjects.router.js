...
subjectsRouter.get('/:id', requireAuth, async (req, res, next) => {
  try {
    const userId = req.user.id;
    const subject = await subjectsService.getSubject(req.params.id, userId);
...
subjectsRouter.patch('/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    const subject = await subjectsService.updateSubject(req.params.id, userId, data);
    res.json({ success: true, data: subject });
  } catch (error) {
    next(error);
  }
...
subjectsRouter.delete('/:id', requireAuth, async (req, res, next) => {
  try {
    const userId = req.user.id;
    await subjectsService.deleteSubject(req.params.id, userId);
